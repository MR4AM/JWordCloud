...
(function (global, factory) {
  ...
  (global = global || self, global.JWordCloud = factory());
  ...
  */
  var JWordCloud = /*#__PURE__*/function () {
    ...
    function JWordCloud() {
      ...
      _classCallCheck(this, JWordCloud);

      var self = this;
      if (!container || container.nodeType !== 1) return new Error('Incorrect element type'); // params

      self.$container = container;
      self.texts = Object.keys(texts) || [];
      self.wordValuesList = Object.values(texts) || [];
      self.config = _objectSpread2(_objectSpread2({}, JWordCloud._defaultConfig), options || {}); // calculate config
      ...
      self.maxSpeed = JWordCloud._getMaxSpeed(self.config.maxSpeed); // rolling max speed

      self.initSpeed = JWordCloud._getInitSpeed(self.config.initSpeed); // rolling init speed
    ...
    _createClass(JWordCloud, [{
      ...
      value: function _init() {
        ...
        JWordCloud._on(self.$el, 'mouseover', function () {
          self.active = true;
        }); // mouseout


        JWordCloud._on(self.$el, 'mouseout', function () {
          self.active = false;
        }); // mousemove


        JWordCloud._on(self.keep ? window : self.$el, 'mousemove', function (ev) {
          ev = ev || window.event;
      ...
      value: function destroy() {
        var self = this;
        self.interval = null; // clear in JWordCloud.list

        var index = JWordCloud.list.findIndex(function (e) {
          return e.el === self.$el;
        });
        if (index !== -1) JWordCloud.list.splice(index, 1); // clear element
  ...
  JWordCloud._getInitSpeed = function (name) {
    return {
      slow: 16,
      normal: 32,
      fast: 80
  ...
  var index = (function (els, texts, options) {
    ...
    els.forEach(function (el) {
      if (el) {
        instances.push(new JWordCloud(el, texts, options));
      }
    });
